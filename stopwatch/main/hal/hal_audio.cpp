...
public:
    ...
    void setSampleRate(int rate)
    {
        ...
        mclog::tagInfo(_tag, "sample rate changed to {}", rate);
    }

    void writeRaw(const int16_t* data, size_t samples)
    {
        std::lock_guard<std::mutex> lock(_mutex);
        esp_codec_dev_write(_codec_dev, (void*)data, samples * sizeof(int16_t));
    }

    int getRate() const { return _active_rate; }
...
void Hal::setAudioSampleRate(int sampleRate)
{
    mclog::tagInfo(_tag, "setAudioSampleRate({})", sampleRate);
    _audio_codec.setSampleRate(sampleRate);
}

void Hal::audioWriteRaw(const int16_t* data, size_t samples)
{
    _audio_codec.writeRaw(data, samples);
}
